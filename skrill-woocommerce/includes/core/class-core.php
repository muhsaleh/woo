...
class SkrillPayment
{
    protected static $prepareUrl = 'https://pay.skrill.com';
    protected static $queryUrl = 'https://www.moneybookers.com/app/query.pl';
    protected static $refundUrl = 'https://www.moneybookers.com/app/refund.pl';
     ...
     * return : array
     */
    public static function getPaymentMethodByType($paymentType)
    {
     ...
     * return : string 
     */
    public static function getPaymentUrlByCheckoutParameters($checkoutParameters)
    {
        $url = self::$prepareUrl;
        $postResult = self::post($url, $checkoutParameters);

        if (isset($postResult)) {
            return $paymentUrl = $url.'?sid='.$postResult;
        }
     ...
     */
    public static function isPaymentAccepted($paymentParameters, &$paymentResult)
    {
        // check status_trn 3 times if no response.
        for ($i=0; $i < 3; $i++) {
            $response = true;
            try {
                $paymentResult = self::getPaymentResult($paymentParameters);
            } catch (Exception $e) {
                $response = false;
            }
     ...
     * return : array
     */
    public static function getPaymentResult($paymentParameters)
    {
        $queryUrl = self::$queryUrl;
        $postResult = self::post($queryUrl, $paymentParameters);

        if(isset($postResult)) {
            return self::setPaymentResultToArray($postResult);
        }
     ...
     * Skrill Refund
     *
     * param : $action(prepare/refund), $parameters
     * return : array
     */
    public static function doRefund($transactionParameters)
    {
        $refundUrl = self::$refundUrl;
        
        $prepareParameters = $transactionParameters;
        $prepareParameters['action'] = 'prepare';
        $postResult = self::post($refundUrl, $prepareParameters);
        ...
        if (isset($sid)) {
            ...
            $postResult = self::post($refundUrl, $refundParameters);     
     ...
     * return : string
     */
    protected static function post($url, $postParameters)
    {
     ...
     * return : array
     */
    public static function setPaymentResultToArray($paymentResult)
    {
        $paymentResultInArray = array();
        $resultString = explode("\n", $paymentResult);
        $paymentResultInArray['response_header'] = $resultString[0];
        $results = explode("&", $resultString[1]);
        foreach ($results as $result) {
            $resultValue = explode("=", $result);
            $paymentResultInArray[urldecode($resultValue[0])] = urldecode($resultValue[1]);
        }
        return $paymentResultInArray;
    }
     ...
     * return : string
     */
    protected static function generateMd5sig($skrillSettings, $paymentResult)
    {
        $merchantId = $skrillSettings['merchant_id'];
        $secretWord = strtoupper($skrillSettings['secret_word']);
        $transactionId = $paymentResult['transaction_id'];
        $mbAmount = $paymentResult['mb_amount'];
        $mbCurrency = $paymentResult['mb_currency'];
    ...
    */
    ...
    public static function getSupportedPaymentsByCountryCode($countryCode)
    {
        ...
        $supportedCountry = self::isSupportedCountry($countryCode);
        $supportedPayments = array();
        if ($supportedCountry) {
            foreach (self::$paymentMethods as $key => $paymentMethod) {
                if ($paymentMethod['countries'] == 'ALL') {
                    $supportedPayments[] = $key;
                } else {
                    $countryList = explode(',', $paymentMethod['countries']);
     ...
     * Supported Country
     */
    public static function isSupportedCountry($countryCode)
    {
        $unsupportedCountries = array('AFG','MMR','NGA','PRK','SDN','SYR','SOM','YEM');
        if (in_array($countryCode, $unsupportedCountries)) {
              return false;
        } 
        return true;
    }
